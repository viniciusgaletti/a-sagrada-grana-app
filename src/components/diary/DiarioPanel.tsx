...
export function DiarioPanel({ userId, date, isOpen, onClose, onChanges }: DiarioPanelProps) {
  const { transactions, total, isLoading, error, create, update, remove, refetch } = useDiarioPanel(
    userId,
    date,
  )
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {isFormOpen && (
            <form
              onSubmit={handleSave}
              className="p-4 rounded-2xl animate-in slide-in-from-bottom-4"
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label
                    className="block text-xs font-sans mb-1"
                    style={{ color: 'var(--muted-foreground)' }}
                  >
                    Valor
                  </label>
